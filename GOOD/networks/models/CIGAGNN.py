...
class CIGAGIN(GNNBasic):
    ...
    def __init__(self, config: Union[CommonArgs, Munch]):
        ...
        if self.contrast_rep == "raw":
            ...
            # nn.Sequential(self.att_net.gnn_node,self.att_net.gnn_node.encoder.readout)
        else:
    ...
    def forward(self, *args, **kwargs):
        ...
        if self.contrast_rep == "raw":
            ...
            causal_rep = self.feat_encoder(data=data)
            causal_rep = self.feat_encoder.encoder.readout(causal_rep, data.batch, batch_size)
        else:
        ...
        if self.training:
            if self.contrast_rep == "raw":
                ...
                # --- Conf repr ---
                set_masks(1-pred_edge_weight, self)
                # spu_rep = self.feat_encoder(data=Data(x=spu_x, edge_index=spu_edge_index,
                #         edge_attr=spu_edge_attr, batch=spu_batch))
                # spu_rep = self.feat_encoder.encoder.readout(spu_rep, spu_batch, batch_size)
                spu_rep = self.feat_encoder(data=data)
                spu_rep = self.feat_encoder.encoder.readout(spu_rep, data.batch, batch_size)
